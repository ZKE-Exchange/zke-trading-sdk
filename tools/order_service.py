...
def test_order(private_api, registry, symbol, side, order_type, volume, price):
...
def create_order(private_api, registry, symbol, side, order_type, volume, price):
...
def cancel_order(private_api, registry, symbol, order_id):

    api_symbol = registry.get_api_symbol(symbol)

    return private_api.cancel_order(api_symbol, order_id=order_id)


def open_orders(private_api, registry, symbol):

    api_symbol = registry.get_api_symbol(symbol)

    return private_api.open_orders(api_symbol)